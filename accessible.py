...
bz2 = ptch.RegularPolygon(
    (0, 0), 6, np.linalg.norm(
        (2 * b1 + b2) / 3), pi / 6, fill=False,color='black')
bz3 = ptch.RegularPolygon((0, 0), 6, np.linalg.norm(b1 + b2), 0, fill=False,color='black')
...
# K= R2(pi/3).dot(K12)
# ha = R2(pi/3).dot(2*K12+np.array([1 / 2, 1/2]).dot([b1, b2]))
#
...
# wowdude = np.array([M, M23, M43, K12, Kp12, L1, L2,K,ha])
# wowdude = np.array([K12, Kp12])
# gggg.axes[0].quiver(*origin, wowdude[:, 0], wowdude[:, 1], color="r",
                    # angles='xy', scale_units='xy', scale=1)
# plt.plot()

gggg.axes[0].set_yticklabels([])
gggg.axes[0].set_xticklabels([])
gggg.axes[0].xaxis.set_tick_params(size=0)
gggg.axes[0].yaxis.set_tick_params(size=0)
plt.show()
plt.savefig("swag.pdf")
plt.close()
